...
function FileBasedSequentialLogStore:new(logContainer)
  ...
  o.buffer =
    LogBuffer:new(
    (o.entriesInStore > o.bufferSize and (o.entriesInStore + o.startIndex - o.bufferSize)) or o.startIndex,
    o.bufferSize
  )
...
function FileBasedSequentialLogStore:writeAt(logIndex, logEntry)
  self.logger.trace(
    "writeAt>logIndex:%d, self.startIndex:%d, self.entriesInStore:%d",
    logIndex,
    self.startIndex,
    self.entriesInStore
  )
...
function FileBasedSequentialLogStore:getLogEntries(startIndex, endIndex)
  self.logger.trace(
    "getLogEntries:startIndex:%d, endIndex:%d, self.startIndex:%d, self.entriesInStore:%d",
    startIndex,
    endIndex,
    self.startIndex,
    self.entriesInStore
...
function FileBasedSequentialLogStore:getLogEntryAt(logIndex)
  self.logger.trace(
    "getLogEntryAt>logIndex:%d, self.startIndex:%d, self.entriesInStore:%d",
    logIndex,
    self.startIndex,
    self.entriesInStore
  )
...
function FileBasedSequentialLogStore:packLog(logIndex, itemsToPack)
  self.logger.trace(
    "packLog>logIndex:%d, itemsToPack:%d, self.startIndex:%d, entriesInStore:%d",
    logIndex,
    itemsToPack,
    self.startIndex,
    self.entriesInStore
...
function FileBasedSequentialLogStore:applyLogPack(logIndex, logPack)
  self.logger.trace(
    "applyLogPack>logIndex:%d, self.startIndex:%d, entriesInStore:%d",
    logIndex,
    self.startIndex,
    self.entriesInStore
  )
  ...
  if (file:IsValid()) then
    ...
    -- openFile(self, "rw")
    self.buffer:reset(
      self.entriesInStore > self.bufferSize and self.entriesInStore + self.startIndex - self.bufferSize or
        self.startIndex
    )
    self:fillBuffer()
    file:close()
...
function FileBasedSequentialLogStore:restore()
  ...
  if
    not (ParaIO.CopyFile(self.backupIndexFileName, self.indexFileName, true) and
      ParaIO.CopyFile(self.backupDataFileName, self.dataFileName, true) and
      ParaIO.CopyFile(self.backupStartIndexFileName, self.startIndexFileName, true))
   then
    -- this is fatal...
...
function FileBasedSequentialLogStore:backup()
  ...
  if
    not (ParaIO.CopyFile(self.indexFileName, self.backupIndexFileName, true) and
      ParaIO.CopyFile(self.dataFileName, self.backupDataFileName, true) and
      ParaIO.CopyFile(self.startIndexFileName, self.backupStartIndexFileName, true))
   then
